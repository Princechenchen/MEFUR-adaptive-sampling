...
ns = 80; model.ns=ns;% number of candidate points
model.variables.ns=ns;
model.variables.dim=size(INPUT,2); % dimension number
model.ClassPos = ClassPos;
model.total_iter = 80;       % Number of final sample points
model.n_init=40; % Number of initial sample points
...
for i = 1:model.total_iter-model.n_init
    ...
    xind = ismember(model.candidate0, x_star, 'rows'); xind=find(xind,1);
    f_star = candidatey(xind,:);
...
% mSUR2wvarcs03 method  增加了权重和距离,增加样本点本身方差,增加了分类边界U函数偏好   %%%here%%%
for i = 1:model.total_iter-model.n_init  
